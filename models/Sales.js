...
var Sales = {
    ...
    getAllSalesByVendorWithDetails: function(vendor_cpf, callback) {
        return db.query("select nickname, user_name, customer_cpf, date_time, products_product_code, products_product_version, product_name, quantity, price from sales s join users u on s.customer_cpf = u.cpf join products p on p.id_product = s.products_id_product where s.vendor_cpf = ?", [vendor_cpf], callback);
    },
